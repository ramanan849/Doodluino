...
void drawLevelMenu() {
  ...
  if (state.selectedOption != lastSelection) {
    ...
    tft.fillRect(40, 115, 160, 130, BLACK); // Adjusted to fully cover the menu
...
void initGame() {
  // Doodler starts at x center, and y center
  ...
  // Starting platform details
  state.platformX_start = state.doodlerX; // Align with doodler's x
  state.platformY_start = state.doodlerY + 50; // Positioned below doodler

  state.gameStartedByUser = false;
  ...
  for (int i = 0; i < NUM_PLATFORMS; i++) {
    ...
    if (state.gameLevel == LEVEL_EASY) {
      state.platformDirection[i] = 0; // Stationary
    } else {
...
void checkCollisions() {
  // Check collision with the starting platform
  if (!state.gameStartedByUser && state.doodlerVelocityY >= 0 &&
      state.doodlerX + DOODLER_WIDTH > state.platformX_start &&
      state.doodlerX < state.platformX_start + PLATFORM_WIDTH &&
      state.prevDoodlerY + DOODLER_HEIGHT <= state.platformY_start &&
      state.doodlerY + DOODLER_HEIGHT >= state.platformY_start) {
    state.doodlerVelocityY = JUMP_FORCE;
    state.gameStartedByUser = true; // Game starts on first jump
  }
...
void handleScrolling() {
  ...
  if (state.doodlerY < SCREEN_HEIGHT / 3) {
    ...
    // Scroll the starting platform
    state.platformY_start += scroll;
...
void drawGame() {
  ...
  // Clear previous starting platform position
  tft.fillRect(state.platformX_start, state.platformY_start + TOP_OFFSET - 50 ,
...
void handleGameOver() {
  ...
  playMusic();
  tft.print("Press joystick (any button)");
