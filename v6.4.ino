...
#define BOTTOM_OFFSET 20  // Height of bottom info panel
...
#define PLAY_AREA_HEIGHT (SCREEN_HEIGHT - TOP_OFFSET - BOTTOM_OFFSET)
...
void setup(); // Modified
void loop(); // Replaced
void drawMainMenu(); // New
void drawLevelMenu(); // Replaced (New implementation)
void drawHighScores(); // New
void drawCredits(); // New
void readMenuInput(); // Replaced (New implementation)
void handleMenuSelection(); // New
void readMenuInput_GameOver(); // New
void resetDrawFlags(); // New

// EEPROM (Original Function)
...
// Other
void handleGameOver(); // Adapted content from original
void playMusic();      // Original Function
...
void checkGameOver() {
  if (state.doodlerY > PLAY_AREA_HEIGHT) // Check against new play area height
    state.gameOver = true;
}

int getClampedDoodlerY() {
  // Calculate maximum Y position where full doodler is visible
  const int maxVisibleY = (SCREEN_HEIGHT - BOTTOM_OFFSET) - DOODLER_HEIGHT;
  const int screenY = state.doodlerY + TOP_OFFSET;
  
  // Return either current position or clamped position
  return min(screenY, maxVisibleY);
}

void drawGame() {
  // --- Helper function with parameter ---
  auto getClampedScreenY = [](int gameY) {
    const int maxVisibleY = (SCREEN_HEIGHT - BOTTOM_OFFSET) - DOODLER_HEIGHT;
    const int screenY = gameY + TOP_OFFSET;
    return min(screenY, maxVisibleY);
  };

  // --- Clear Previous Frame ---
  // 1. Clear previous platforms FIRST
  for (int i = 0; i < NUM_PLATFORMS; i++) {
    if (state.platformX[i] != state.prevPlatformX[i] ||
        state.platformY[i] != state.prevPlatformY[i]) {
      tft.fillRect(state.prevPlatformX[i], 
                  getClampedScreenY(state.prevPlatformY[i]),
                  PLATFORM_WIDTH, 
                  PLATFORM_HEIGHT, 
                  BLACK);
    }
  ...
  // 2. Clear previous Doodler using CORRECT previous position
  tft.fillRect(state.prevDoodlerX, 
              getClampedScreenY(state.prevDoodlerY),
              DOODLER_WIDTH, 
              DOODLER_HEIGHT, 
              BLACK);

  // --- Draw New Frame ---
  // 1. Draw platforms FIRST
  for (int i = 0; i < NUM_PLATFORMS; i++) {
    if (i < state.visiblePlatforms && 
        state.platformY[i] + TOP_OFFSET >= 0 &&
        state.platformY[i] + TOP_OFFSET <= (SCREEN_HEIGHT - BOTTOM_OFFSET)) {
      tft.fillRect(state.platformX[i], 
                  getClampedScreenY(state.platformY[i]),
                  PLATFORM_WIDTH, 
                  PLATFORM_HEIGHT, 
                  GREEN);
    }
  ...
  // 2. Draw Doodler LAST (on top of platforms)
  tft.fillRect(state.doodlerX, 
              getClampedScreenY(state.doodlerY),
              DOODLER_WIDTH, 
              DOODLER_HEIGHT, 
              WHITE);

  // ... rest of score panel and debug drawing ...
}
void handleGameOver() {
...
// === REMOVED Original v5 Functions ===
/*
void RestartGame(){ // REMOVED
  // ... original content ...
}

void showStartScreen() { // REMOVED
  // ... original content ...
}

void drawLevelMenu() { // REMOVED (Replaced with new version)
  // ... original content ...
}

void readMenuInput() { // REMOVED (Replaced with new version)
  // ... original content ...
}
*/