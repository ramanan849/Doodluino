...
void drawCredits() {
  ...
  tft.print("Copyright LUKOG");
  
  // Back button
  tft.fillRoundRect(80, 220, 80, 30, 5, RED);
...
void drawHighScores() {
  ...
  tft.setTextSize(2);
  tft.setCursor(30, 50);
  tft.print("ALL TIME HIGH SCORES");
  
  tft.setCursor(60, 100);
  tft.print("EASY: ");
  tft.print(highScore.easy_score);
  ...
  tft.print("HARD: ");
  tft.print(highScore.hard_score);
...
void showMainMenu() {
  ...
  while(true) {
    ...
    // Check if we need to exit back to main menu
    if(state.currentMenu == MAIN_MENU) {
      drawMainMenu();
    }
    else {
      // Handle submenu screens
      switch(state.currentMenu) {
        case LEVEL_MENU:
          drawLevelMenu();
          while(state.currentMenu == LEVEL_MENU) {
            readMenuInput();
          }
          break;
        case HIGHSCORES_MENU:
          drawHighScores();
          while(state.currentMenu == HIGHSCORES_MENU) {
            readMenuInput();
          }
          break;
        case CREDITS_MENU:
          drawCredits();
          while(state.currentMenu == CREDITS_MENU) {
            readMenuInput();
          }
          break;
      }
...
void readMenuInput() {
  ...
  int maxOption = 2; // Default for main menu

  if(state.currentMenu == LEVEL_MENU) maxOption = 3;
  else if(state.currentMenu == HIGHSCORES_MENU) maxOption = 0;

  if(yValue < 400) {
    if(state.currentMenu == MAIN_MENU) {
      state.mainMenuSelection = min(state.mainMenuSelection + 1, 2);
    } else {
      state.selectedOption = min(state.selectedOption + 1, maxOption);
    }
    lastInputTime = millis();
  } 
  else if(yValue > 600) {
    if(state.currentMenu == MAIN_MENU) {
      state.mainMenuSelection = max(state.mainMenuSelection - 1, 0);
    } else {
...
void handleMenuSelection() {
  if(state.currentMenu == MAIN_MENU) {
    switch(state.mainMenuSelection) {
      case 0: // Level Select
        state.currentMenu = LEVEL_MENU;
        state.selectedOption = 0;
        break;
      case 1: // High Scores
        state.currentMenu = HIGHSCORES_MENU;
        break;
      case 2: // Credits
  ...
  else if(state.currentMenu == LEVEL_MENU) {
  ...
  else if(state.currentMenu == HIGHSCORES_MENU || state.currentMenu == CREDITS_MENU) {
    // Check if user clicked BACK button (you'll need to add position checks)
    if(digitalRead(JOY_SW) == LOW) {
      state.currentMenu = MAIN_MENU;
    }
  }
