...
void updateObstacles() {
  // Spawn new obstacles only when score threshold is reached
  if (state.score >= OBSTACLE_START_SCORE && random(100) < OBSTACLE_SPAWN_CHANCE) {
    for (int i = 0; i < MAX_OBSTACLES; i++) {
      if (!state.obstacleActive[i]) {
        ...
        if (horizontalDifference > 5) { // Doodler is to the right (added a small deadzone of 5 pixels)
        state.obstacleX[i] += HOMING_FACTOR;
        } 
        else if (horizontalDifference < -5) { // Doodler is to the left (added a small deadzone of 5 pixels)
            state.obstacleX[i] -= HOMING_FACTOR;
        }
        if (state.obstacleX[i] <= OBSTACLE_RADIUS ||
              state.obstacleX[i] >= SCREEN_WIDTH - OBSTACLE_RADIUS) {
            state.obstacleDir[i] *= -1;
            // Optional: Clamp position to prevent getting stuck exactly at boundary if speed+homing is large
            state.obstacleX[i] = constrain(state.obstacleX[i], OBSTACLE_RADIUS, SCREEN_WIDTH - OBSTACLE_RADIUS);
          }

        state.obstacleY[i] = random(-PLATFORM_HEIGHT+20, 0); // Spawn above screen
